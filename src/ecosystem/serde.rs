...
macro_rules! __js_serializable_serde_boilerplate {
    (($($impl_arg:tt)*) ($($kind_arg:tt)*) ($($bounds:tt)*)) => {
        ...
        impl< $($impl_arg),* > $crate::private::JsSerializable for $($kind_arg)* where $($bounds)* {
            #[inline]
            fn into_js< 'x >( &'x self, arena: &'x $crate::private::PreallocatedArena ) -> $crate::private::SerializedValue< 'x > {
                let value = $crate::private::to_value( self ).unwrap();
                let value = arena.save( value );
                $crate::private::JsSerializable::into_js( value, arena )
            }
...
impl< T: Serialize > JsSerializable for Serde< T > {
    #[inline]
